...
bundle_file="$("${CONTAINER_CLI}" exec "${container_id}" sh -c \
    'grep -l "fetch(\"/license.txt\")" /app-root/_master_app/*.js | head -n 1')"
if [[ -z "${bundle_file}" ]]; then
    echo "Explorer Terms bundle not found" >&2
    exit 1
fi
...
if [[ "${bundle_response}" != *'fetch("license.txt")'* ]]; then
    echo "Explorer Terms request is not relative to the ingress base" >&2
    exit 1
fi

index_response="$("${CONTAINER_CLI}" exec "${container_id}" \
    curl --max-time 2 --silent --show-error --fail \
    http://127.0.0.1:18081/)"
if [[ "${index_response}" != *"<base href=\"${INGRESS_ENTRY}/\">"* ]]; then
    echo "Explorer document does not set the ingress base" >&2
    exit 1
fi

if ! "${CONTAINER_CLI}" exec "${container_id}" sh -c \
    'curl --max-time 2 --silent --show-error --fail http://127.0.0.1:18081/license.txt | cmp - /app-root/license.txt'; then
    echo "Explorer did not serve the original Terms document" >&2
    exit 1
fi

echo "Explorer Terms document loads through the ingress base"
